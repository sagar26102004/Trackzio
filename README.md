...
- **Region**: Render has no Mumbai region, so the API runs in Singapore against Supabase in `ap-south-1`. Every L2 read pays that hop — still an order of magnitude cheaper than a TMDB call, and L1 absorbs the hot path.
