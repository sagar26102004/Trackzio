...
- **`api/server.ts`** — one function that mounts the whole Express app, with `vercel.json` rewriting every `/api/*` path to it. This started life as `api/[...path].ts`, on the assumption that Vercel's `/api` directory convention expands `[...param]` into a catch-all the way Next.js does. It does not — for a plain Node function it registers as a *single* dynamic segment, so `/api/movies` worked while `/api/movies/550` returned Vercel's own 404 without ever invoking the function. That silently took down every auth route, the wishlist write path and the whole movie detail page, and none of it reproduces locally, where Express does its own routing. The rewrite carries the original path in `__p` so the Express router mounted at `/api` sees the real path regardless of what the platform does to `req.url`.
